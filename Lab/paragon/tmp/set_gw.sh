#!/bin/bash
cat << EOF  | sudo tee /etc/dhcp/dhcpd.conf
default-lease-time 600;
max-lease-time 7200;
ddns-update-style none;
option space NEW_OP;
option NEW_OP.image-file-name code 0 = text;
option NEW_OP.config-file-name code 1 = text;
option NEW_OP.image-file-type code 2 = text; 
option NEW_OP.transfer-mode code 3 = text;
option NEW_OP.alt-image-file-name code 4= text;
option NEW_OP.http-port code 5= text;
option NEW_OP-encapsulation code 43 = encapsulate NEW_OP;
option NEW_OP.proxyv4-info code 8 = text;
option option-150 code 150 = { ip-address };
option domain-name-servers 10.49.32.95, 10.49.32.97;


host pe1 {
    hardware ethernet 56:04:1b:00:0b:e4;
    fixed-address 172.16.10.1;
    option NEW_OP.config-file-name "pe1.conf";
    option host-name "pe1";
}
host pe2 {
    hardware ethernet 56:04:1b:00:56:b6;
    fixed-address 172.16.10.2;
    option NEW_OP.config-file-name "pe2.conf";
    option host-name "pe2";
}
host pe3 {
    hardware ethernet 56:04:1b:00:86:21;
    fixed-address 172.16.10.3;
    option NEW_OP.config-file-name "pe3.conf";
    option host-name "pe3";
}
host pe4 {
    hardware ethernet 56:04:1b:00:21:11;
    fixed-address 172.16.10.4;
    option NEW_OP.config-file-name "pe4.conf";
    option host-name "pe4";
}
host p1 {
    hardware ethernet 56:04:1b:00:6c:b3;
    fixed-address 172.16.10.11;
    option NEW_OP.config-file-name "p1.conf";
    option host-name "p1";
}
host p2 {
    hardware ethernet 56:04:1b:00:6b:db;
    fixed-address 172.16.10.12;
    option NEW_OP.config-file-name "p2.conf";
    option host-name "p2";
}
host p3 {
    hardware ethernet 56:04:1b:00:6e:7e;
    fixed-address 172.16.10.13;
    option NEW_OP.config-file-name "p3.conf";
    option host-name "p3";
}
host p4 {
    hardware ethernet 56:04:1b:00:4e:da;
    fixed-address 172.16.10.14;
    option NEW_OP.config-file-name "p4.conf";
    option host-name "p4";
}
host p5 {
    hardware ethernet 56:04:1b:00:3e:9f;
    fixed-address 172.16.10.15;
    option NEW_OP.config-file-name "p5.conf";
    option host-name "p5";
}
host client {
    hardware ethernet 56:04:1b:00:80:fa;
    fixed-address 172.16.10.100;
    option NEW_OP.config-file-name "client.conf";
    option host-name "client";
}
host br1 {
    hardware ethernet 56:04:1b:00:1f:08;
    fixed-address 172.16.10.101;
    option NEW_OP.config-file-name "br1.conf";
    option host-name "br1";
}
host br2 {
    hardware ethernet 56:04:1b:00:63:07;
    fixed-address 172.16.10.102;
    option NEW_OP.config-file-name "br2.conf";
    option host-name "br2";
}
host br3 {
    hardware ethernet 56:04:1b:00:2f:45;
    fixed-address 172.16.10.103;
    option NEW_OP.config-file-name "br3.conf";
    option host-name "br3";
}
subnet 172.16.10.0 netmask 255.255.255.0  {
    range 172.16.10.5 172.16.10.100;
    option routers 172.16.10.254;
    option option-150 172.16.10.254;
}
EOF
cat << EOF | sudo tee /etc/netplan/02_net.yaml
network:
  ethernets:
    eth1:
      addresses : ['172.16.10.254/24']
       
    eth2:
      addresses : ['172.16.11.1/24']
       
    eth3:
      addresses : ['172.16.14.0/31']
      routes: 
       - to: 10.100.0.0/24
         via: 172.16.14.1
         metric: 1
       - to: 10.100.1.0/24
         via: 172.16.14.1
         metric: 1
       
       
      
EOF



cat << EOF | sudo tee /usr/local/bin/start_vnc.sh
#!/bin/bash
websockify -D --web=/usr/share/novnc/ 6081 q-pod-80b.englab.juniper.net:5947
websockify -D --web=/usr/share/novnc/ 6082 q-pod-79u.englab.juniper.net:5929
websockify -D --web=/usr/share/novnc/ 6083 q-pod-80i.englab.juniper.net:5922
websockify -D --web=/usr/share/novnc/ 6084 q-pod-79w.englab.juniper.net:5960
websockify -D --web=/usr/share/novnc/ 6085 q-pod-80l.englab.juniper.net:5968
exit 1
EOF

sudo chmod +x /usr/local/bin/start_vnc.sh

cat << EOF | sudo tee /etc/update-motd.d/99-update
#!/bin/bash
echo "----------------------------------------------"
echo "To access console of VM, use the following URL"
echo "----------------------------------------------"
echo "console node1 : http://10.53.102.243:6081/vnc.html"
echo "console node2 : http://10.53.102.243:6082/vnc.html"
echo "console node3 : http://10.53.102.243:6083/vnc.html"
echo "console node4 : http://10.53.102.243:6084/vnc.html"
echo "console client : http://10.53.102.243:6085/vnc.html"
EOF

sudo chmod +x  /etc/update-motd.d/99-update
cat << EOF | tee ~/.ssh/authorized_keys
ssh-rsa  irzan@irzan-mbp
EOF

cat << EOF |  tee ~/.ssh/id_rsa
-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
EOF

chmod og-rwx ~/.ssh/*

sudo netplan apply

sudo systemctl enable startup.service
sudo systemctl restart startup.service
sudo systemctl restart isc-dhcp-server
sudo systemctl restart tftpd-hpa
sudo mv ~/tftp/*.conf /srv/tftp/
# sudo reboot
