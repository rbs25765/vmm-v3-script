#!/bin/ash
cat << EOF | sudo tee /etc/hosts
127.0.0.1   localhost localhost.localdomain localhost4 localhost4.localdomain4
::1         localhost localhost.localdomain localhost6 localhost6.localdomain6
127.0.1.1 br3
EOF

sudo rm /etc/local.d/*

cat << EOF | sudo tee /etc/hostname
br3
EOF

rm -rf ~/.ssh/
mkdir ~/.ssh

cat << EOF | tee ~/.ssh/authorized_keys
ssh-rsa  irzan@irzan-mbp
EOF

cat << EOF | tee ~/.ssh/id_rsa
-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
EOF

sudo mkdir /root/.ssh
sudo cp ~/.ssh/id_rsa /root/.ssh/id_rsa
sudo cp ~/.ssh/authorized_keys /root/.ssh/authorized_keys



cat << EOF | sudo tee /etc/network/interfaces
auto lo
iface lo inet loopback
auto eth0
iface eth0 inet static
   address 172.16.10.103/24
   gateway 172.16.10.254
   dns-nameservers 10.49.32.95 10.49.32.97 
   
 
auto eth1
iface eth1 inet manual
    mtu 9000
 
auto eth2
iface eth2 inet manual
    mtu 9000
 
auto eth3
iface eth3 inet manual
    mtu 9000
 
auto eth4
iface eth4 inet manual
    mtu 9000
 
auto eth5
iface eth5 inet manual
    mtu 9000
 
auto eth6
iface eth6 inet manual
    mtu 9000
 
auto eth7
iface eth7 inet manual
    mtu 9000
 
auto eth8
iface eth8 inet manual
    mtu 9000
 
auto eth9
iface eth9 inet manual
    mtu 9000
 
auto eth10
iface eth10 inet manual
    mtu 9000
 

EOF


uuidgen  | sed -e 's/-//g' |  sudo tee /etc/machine-id

cat << EOF | tee ~/.ssh/config
Host *
   StrictHostKeyChecking no
EOF

cat << EOF | sudo tee /etc/local.d/p3p4.start
#!/bin/sh
ip link add dev p3p4 type bridge
ip link set dev eth1 master p3p4
ip link set dev eth2 master p3p4

ip link set p3p4 up
echo 0x4000 >  /sys/class/net/p3p4/bridge/group_fwd_mask
EOF
sudo chmod +x /etc/local.d/p3p4.start
cat << EOF | sudo tee /etc/local.d/pe3p3.start
#!/bin/sh
ip link add dev pe3p3 type bridge
ip link set dev eth3 master pe3p3
ip link set dev eth4 master pe3p3

ip link set pe3p3 up
echo 0x4000 >  /sys/class/net/pe3p3/bridge/group_fwd_mask
EOF
sudo chmod +x /etc/local.d/pe3p3.start
cat << EOF | sudo tee /etc/local.d/pe3p4.start
#!/bin/sh
ip link add dev pe3p4 type bridge
ip link set dev eth5 master pe3p4
ip link set dev eth6 master pe3p4

ip link set pe3p4 up
echo 0x4000 >  /sys/class/net/pe3p4/bridge/group_fwd_mask
EOF
sudo chmod +x /etc/local.d/pe3p4.start
cat << EOF | sudo tee /etc/local.d/pe4p3.start
#!/bin/sh
ip link add dev pe4p3 type bridge
ip link set dev eth7 master pe4p3
ip link set dev eth8 master pe4p3

ip link set pe4p3 up
echo 0x4000 >  /sys/class/net/pe4p3/bridge/group_fwd_mask
EOF
sudo chmod +x /etc/local.d/pe4p3.start
cat << EOF | sudo tee /etc/local.d/pe4p4.start
#!/bin/sh
ip link add dev pe4p4 type bridge
ip link set dev eth9 master pe4p4
ip link set dev eth10 master pe4p4

ip link set pe4p4 up
echo 0x4000 >  /sys/class/net/pe4p4/bridge/group_fwd_mask
EOF
sudo chmod +x /etc/local.d/pe4p4.start

sudo rc-update add local default


chmod og-rwx ~/.ssh/*

sleep 2 
sudo reboot